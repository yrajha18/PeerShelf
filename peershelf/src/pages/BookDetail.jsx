...
const BookDetail = () => {
  ...
  const fetchBook = async () => {
    ...
    if (!error) {
      setBook(data);
      fetchSellerProfile(data.user_id); // 🔥 Fetch seller profile
    } else {
    ...
    <div className="p-4 sm:p-6 max-w-3xl mx-auto">
      ...
      <div className="bg-white shadow rounded-2xl p-6">
        ...
        <div className="flex items-center gap-3">
          ...
          {sellerProfile && sellerProfile.rating_count > 0 && (
            <div className="flex items-center bg-gray-100 px-2 py-1 rounded text-xs font-medium">
              <span className="text-yellow-500 mr-1">⭐</span>
              {sellerProfile.average_rating} ({sellerProfile.rating_count} reviews)
            </div>
          )}
          {sellerProfile?.is_trusted && (
            <span className="bg-blue-100 text-blue-700 px-2 py-1 rounded text-xs font-bold flex items-center">
              <span className="mr-1">🛡️</span> Trusted Seller
            </span>
          )}
        </div>
